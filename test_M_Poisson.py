...
if torch.cuda.is_available():
    ...
    print("GPU name:", torch.cuda.get_device_name(0))  # Replace 0 with the GPU index if multiple GPUs are available.

    # Set the device to GPU
    device = torch.device("cuda:0")  # Replace 0 with the GPU index if multiple GPUs are available.
...
#best_model = 'pth/model_Bike_DC_60min.pth' #place it with other models
STmodel = torch.load(best_model,map_location=torch.device('cpu')).to(device=device)
#torch.device('cuda') 
...
STmodel = probability_model(time_length, nhid_spatial, dropout, alpha, nheads_spatial,
                temporal_hidden_dim, pred_len, channels, dis_hid,num_mixture, distribution_name)
...
with torch.no_grad():
    ...
    for i in range(0,test_input.shape[0],batch_size):
        ...
        test_loss = STmodel.distribution.loss(y_batch, prob_success,pi)
        
        test_loss = np.asscalar(test_loss.detach().numpy())
        
       
        mu_1 = torch.sum(pi * prob_success,dim = -1)
       
       
        

        mean_pred = mu_1#torch.abs(1-pi_test) *  torch.sum((pi_g_test * torch.exp(mu_g_test)),dim = -1)

        test_pred_all[i:i+batch_size] = mean_pred
        test_pred_all_var[i:i+batch_size] = torch.sqrt(torch.sum(pi*pi*prob_success,-1))
