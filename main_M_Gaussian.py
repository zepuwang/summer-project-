...
for epoch in range(epochs):
    ...
    for i in range(0, training_input.shape[0], batch_size):
        ...
        mu_1 = torch.sum(mu, dim=-1)

        MSE = nn.MSELoss()

        loss = STmodel.distribution.loss(y_batch, pi, mu, sigma) + MSE(
            y_batch, torch.sum(pi * mu, -1)
        )
        loss.backward()
        optimizer.step()
        total_loss += loss
    ...
    for i in range(0, val_input.shape[0], batch_size):
        with torch.no_grad():
            ...
            val_loss = STmodel.distribution.loss(y_batch, pi, mu, sigma) + MSE(
                y_batch, torch.sum(pi * mu, -1)
            )
            sub_validation_nll.append(val_loss.detach().cpu().numpy())

            val_pred = torch.sum(pi * mu, dim=-1)
    ...
    Save the model
    """
    if validation_nll[-1] == min(validation_nll):
